...
include(PROJECT_ROOT * "/src/modules/reciprocal-space-algorithm-d-wave.jl")
...
magnetization) and `Δn` (tolerance on density in chemical potential estimation),
`g` (mixing parameter). It performs an iterative HF analysis over a sequence of
2D square lattices for all the possible combinations of the specified 
parameters. Check the source files in the `/modules` folder for more
...
function RunHFRoutine(
    ...
    if FilePathOut != ""
       Header = "# U, V, Lx, β, δ, m, Q, ΔT [calculated @ $(now())]\n"
    ...
    ResultsTable = zeros( length(UU) * length(VV) * length(LL) * length(ββ) * 
    	length(δδ), 8 )
    i = 1    
    for (u,U) in enumerate(UU), (v,V) in enumerate(VV), Lx in LL, β in ββ, δ in δδ
        L = [Lx, Lx]
        printstyled(
            "\e[2K\e[1GRunning HF for U=$U, V=$V, L=$Lx, β=$β, δ=$δ", 
            color=:yellow
        )
        ResultsTable[i,1:5] .= [U,V,Lx,β,δ]
        ResultsTable[i,6:8] .= RunHFAlgorithm(U,V,L,0.5+δ,β,p,Δm,Δn,g)
        i += 1
    end
...
function main()
    ...
    RunHFRoutine(UU,VV,LL,δδ,ββ,p,Δm,Δn,g;FilePathOut)
end
