...
\begin{figure}
	...
	\caption{Pseudo-magnetic field originating from mean-field treatment of the square Hubbard hamiltonian.}
...
\[
	...
	\end{bmatrix}
\]
The hamiltonian represents a magnetic field tilted by an angle $\tan(2\theta_{\mathbf{k}\sigma}) = \Delta_\sigma/\epsilon_{\mathbf{k}}^{(0)}$, as sketched in Fig.~\ref{appfig:pseudo-magnetic-field}. Diagonalization of each $h_{\mathbf{k}\sigma}$ is obtained trivially by a simple rotation around the $y$ axis:
\[
...
\]
Next section describes a viable HF algorithm.
...
\[
\begin{aligned}
	m &= \frac{1}{N^2} \sum_\mathbf{r} \left\langle
		\hat n_{\mathbf{r}\uparrow} - \hat n_{\mathbf{r}\downarrow}
	\right\rangle \\
	&= \frac{1}{N^2} \sum_{\mathbf{k} \in \mathrm{BZ}} \left\langle
		\hat n_{\mathbf{k}\uparrow} - \hat n_{\mathbf{k}\downarrow}
	\right\rangle \\
	&= \frac{1}{N^2} \sum_{\mathbf{k} \in \mathrm{MBZ}} \left\langle
		\left(
			\hat n_{\mathbf{k}\uparrow} + \hat n_{\mathbf{k}+\bm{\pi}\uparrow}
		\right) - \left( 
			\hat n_{\mathbf{k}\downarrow} + \hat n_{\mathbf{k}+\bm{\pi}\downarrow}
		\right)
	\right\rangle
\end{aligned}
\]
...
\begin{aligned}
	...
	\right\rangle \\
	&= \left\langle 
		\hat \Phi_{\mathbf{k}\sigma}^\dagger W_{\mathbf{k}\sigma} W_{\mathbf{k}\sigma}^\dagger \hat \Phi_{\mathbf{k}\sigma}
	\right\rangle \\
	&= \left\langle 
		\hat \Phi_{\mathbf{k}\sigma}^\dagger \hat \Phi_{\mathbf{k}\sigma}
	\right\rangle
\end{aligned}
\]
In last passage, orthogonality of the $W$ matrices has been used. Then magnetization can be computed simply by
\begin{equation}\label{appeq:antiferromagnet-magnetization-self-consistence}
	m = \frac{1}{N^2} \sum_{\mathbf{k} \in \mathrm{MBZ}} \left[
		\left\langle 
			\hat \Phi_{\mathbf{k}\uparrow}^\dagger \hat \Phi_{\mathbf{k}\uparrow}
		\right\rangle - \left\langle 
			\hat \Phi_{\mathbf{k}\downarrow}^\dagger \hat \Phi_{\mathbf{k}\downarrow}
		\right\rangle
	\right]
\end{equation}
...
\begin{enumerate}
	...
	\begin{enumerate}
		\item the coarse-graining of the $\mathrm{BZ}$ -- which is, fix $N_x$ and $N_y$;
		\item the number of iterations $k \in \mathbb{N}$; 
	...
	\item Initialize each $h_{\mathbf{k}\sigma}$ matrix according to the initialized $m_0$;
	\item Diagonalize each $h_{\mathbf{k}\sigma}$ matrix collecting the $\Phi$ eigenvectors;
	\item Compute $m$ using Eq.~\eqref{appeq:antiferromagnet-magnetization-self-consistence};
