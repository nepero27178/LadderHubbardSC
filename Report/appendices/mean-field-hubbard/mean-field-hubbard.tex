...
\subsection{Reciprocal space mean-field iterative algorithmic solution}
...
\begin{enumerate}
	...
	\end{enumerate}
	\item Diagonalize the matrix associated to the operator $\hat H_{\alpha\beta}-\mu \hat N$ collecting the $\mathbf{v}^\ell$ eigenvectors;
	\item Compute $m$ using Eq.~\eqref{appeq:average-magnetization-hf-expression} and update the counter, $i \to i+1$;
