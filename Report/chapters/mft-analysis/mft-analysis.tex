...
\begin{equation}\label{eq:af-fock-hop-ss-renormalization}
	...
	\rangle
\end{equation}
In principle, $\tilde{t}$ should be $\sigma$-dependent, since the spin label appears on the right-hand side of Eq.~\eqref{eq:af-fock-hop-ss-renormalization}. The full effective MFT hamiltonian is however spin-independent, {\color{tabred}then similarly the renormalized parameters cannot exhibit spin dependency. This justifies the disappearing of the spin index on the left-hand side of Eq.~\eqref{eq:af-fock-hop-ss-renormalization}.}

Of course, the quantity $\langle \hat c_{\mathbf{k}\sigma}^\dagger  \hat c_{\mathbf{k}\sigma} \rangle$ of Eq.~\eqref{eq:af-fock-hop-ss-renormalization} needs to be computed self-consistently under the HF general procedure. From Eq.~\eqref{appeq:finite-temperature-order-parameter-derivation-intermediate}, if $\mathbf{k}\in\mathrm{MBZ}$:
\begin{align}
	\langle
...
\begin{equation}\label{eq:af-fock-gap-ss-renormalization-intermediate}
	\tilde{\Delta}_{\mathbf{k}\sigma} \equiv m(U + 2zV) \times (-1)^{\delta_{\sigma=\uparrow}} + 2V w_\sigma \left[
		\cos \left(
			k_x
		\right)	+ \cos \left(
			k_y
...
where
\[
	w_\sigma \equiv \frac{1}{L_xL_y} \sum_{\mathbf{k}'}
	\left[
		\cos \left(
			k_x'
		\right)	+ \cos \left(
	...
	\rangle
\]
Now, it would be nice to have $w_\uparrow=-w_\downarrow\equiv w$. This way we could reduce $\tilde{\Delta}_{\mathbf{k}\uparrow}=-\tilde{\Delta}_{\mathbf{k}\downarrow}\equiv\tilde{\Delta}_\mathbf{k}$. This is the case, for a simple symmetry reason: the AF phase does not exhibit global polarization, which means, no many-body physical quantity should depend on $\sigma$. This applies to the gapped bands: $\tilde{E}_{\mathbf{k}\uparrow}=\tilde{E}_{\mathbf{k}\downarrow}=\tilde{E}_\mathbf{k}$. Since
\[
	\tilde{E}_\mathbf{k} = \sqrt{
		\tilde{\epsilon}_\mathbf{k}^2 + |\tilde{\Delta}_{\mathbf{k}\sigma}|^2
	}
\]
this implies necessarily $|\tilde{\Delta}_{\mathbf{k}\uparrow}| = |\tilde{\Delta}_{\mathbf{k}\downarrow}|$. When it comes to $w_\sigma$, since $\tilde{\Delta}_{\mathbf{k}\sigma}$ contains the ``$m$'' contribution -- independent of $\mathbf{k}$ -- the condition specified above is only possible if actually $w_\uparrow=-w_\downarrow\equiv w$. This then gives us the final result for the renormalized gap function,
\begin{equation}\label{eq:af-fock-gap-ss-renormalization}
	...
	\right]
\end{equation}
This result, together with Eqns.~\eqref{eq:af-hartree-chemical-potential-renormalization} and \eqref{eq:af-fock-hop-ss-renormalization}, concludes the renormalization of all parameters due to the non-local interaction.
{\color{tabred}[
	Surely there is a more elegant way of finding this result.	
]}
