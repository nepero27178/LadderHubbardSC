...
App.~\mref{} describes in detail the MFT treatment of the pure Hubbard model, $\hat H_t + \hat H_U$; the key passage is there given by the approximation
...
\end{itemize}
Then, to look for AF instability only the Hartree term is to be accounted; instead, in superconducting instability only the Cooper term contributes. The Fock term we always assume to be suppressed, preserving $\mathrm{SU}(2)$ symmetry. Note that for superconducting instabilities, due to superexchange mechanism (as explained in App.~\mref{}) the o.s. term account for singlet pairing as well as zero-spin triplet pairing. Which channel is preferred, is a matter of thermodynamic advantage.
