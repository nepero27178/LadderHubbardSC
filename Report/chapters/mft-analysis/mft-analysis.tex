...
\begin{figure}
	...
	\subfloat[
		...
		\label{subfig:af-bz-fock-terms-nonzero}
		\input{chapters/mft-analysis/pictures/af-bz-fock-terms-nonzero}
	}
...
\[
	...
	\right]	(n-4m_\sigma)
\]
{\color{tabred}[I am pretty sure the size denominator should not be there...]}
...
%	\right]
%\]
%and, restricting to the MBZ,
%\begin{multline}\label{eq:reciprocal-space-non-local-interaction-fock-intermediate-2}
...
%				\hat c_{-\mathbf{k} \sigma}
%			\rangle
%			\hat c_{\mathbf{k} \sigma}^\dagger  \hat c_{\mathbf{k}\sigma} + \langle
...
%
%\paragraph{Diagonal terms.}
%The diagonal terms of Eq.~\eqref{eq:reciprocal-space-non-local-interaction-fock-intermediate-2} are treated by the means of Eq.~\eqref{appeq:finite-temperature-order-parameter-derivation-intermediate}. Given $\mathbf{k}\in\mathrm{MBZ}$,
%\[
%\begin{aligned}
%	\langle
%		\hat c_{-\mathbf{k} \sigma}^\dagger 
...
%		W_{-\mathbf{k}\sigma}
%	]_{11}|^2 f(
%		-\tilde{E}_{-\mathbf{k}\sigma}; \beta,\mu
%	) + |[
%		W_{-\mathbf{k}\sigma}
%	]_{21}|^2 f(
%		\tilde{E}_{-\mathbf{k}\sigma}; \beta,\mu
%	) \\
%	&= \sin^2 \theta_{-\mathbf{k}\sigma} f\left(
%		-\tilde{E}_{-\mathbf{k}\sigma}; \beta,\mu
%	\right) + \cos^2 \theta_{-\mathbf{k}\sigma} f\left(
%		\tilde{E}_{-\mathbf{k}\sigma}; \beta,\mu
%	\right) \\
%	&= \frac{1 - \cos (2\tilde{\theta}_{-\mathbf{k}\sigma})}{2} f\left(
%		-\tilde{E}_{-\mathbf{k}\sigma}; \beta,\mu
%	\right) + \frac{1 + \cos (2\tilde{\theta}_{-\mathbf{k}\sigma})}{2} f\left(
%		\tilde{E}_{-\mathbf{k}\sigma}; \beta,\mu
%	\right) \\
%	&= \frac{1}{2} \left[
%		\left(
%			1 - \frac{\tilde{\epsilon}_{-\mathbf{k}\sigma}}{\tilde{E}_{-\mathbf{k}\sigma}} 
%		\right) f \left(
%			-\tilde{E}_{-\mathbf{k}\sigma}; \beta,\mu
%		\right) + \left(
%			1 + \frac{\tilde{\epsilon}_{-\mathbf{k}\sigma}}{\tilde{E}_{-\mathbf{k}\sigma}} 
...
%		\hat c_{-(\mathbf{k}+\bm{\pi})\sigma}^\dagger 
%		\hat c_{-(\mathbf{k}+\bm{\pi})\sigma}
%	\rangle = \frac{1}{2} \Bigg[
%		\left(
%			1 - \frac{\tilde{\epsilon}_{-(\mathbf{k}+\bm{\pi})\sigma}}{\tilde{E}_{-(\mathbf{k}+\bm{\pi})\sigma}} 
%		\right) f\left(
%			-\tilde{E}_{-(\mathbf{k}+\bm{\pi})\sigma}; \beta,\mu
%		\right) \\
%		+ \left(
%			1 + \frac{\tilde{\epsilon}_{-(\mathbf{k}+\bm{\pi})}\sigma}{\tilde{E}_{-(\mathbf{k}+\bm{\pi})\sigma}} 
%		\right) f\left(
...
%	\Bigg]
%\end{multline*}
%\todo {\color{tabred} Check the code here below for a commented section using a different approach, and imposing the reciprocal Ansatz instead of the standard self-consistent Nambu approach. Still unclear if this procedure makes sense or is actually useful.}
