...
\section{Antiferromagnetic phase}
...
\]
(superscript ``$(0)$'' has been dropped momentarily).
...
\begin{figure}
	...
	\caption{Depiction of the AF bands $\pm E_\mathbf{k}$}
	\label{}
\end{figure}
...
\subsection{Instability of the commensurate AF solution and atypical metallic bands}
...
\]
As is shown in \cite{singh1990collective}, for the commensurate AF SSB at wavevector $\bm{\pi}$ the condition for the presence of a stable gapless mode reduces to a $2\times 2$ matrix equation,
\[
	\mathbb{I} - U \left[\chi_{\hat{\mathbf{S}}^- \hat{\mathbf{S}}^+}(\bm{\pi},0)\right] \ge 0
\]
with $\left[\chi_{\hat{\mathbf{S}}^- \hat{\mathbf{S}}^+}(\bm{\pi},\omega)\right]$ the response function matrix obtained by considering a matrix response function for a two-sites unit cell. Considering the eigenvalue of largest amplitude of the matrix $\lambda_\delta$ at filling $\delta$, the one most prone to instabilities, it is found that
\[
\begin{aligned}
...
\end{aligned}
\]
For any finite doping, the commensurate AF phase becomes unstable and is destroyed by transverse spin fluctuations.

\subsection{Hartree-Fock algorithm sketch in reciprocal space}\label{appsubsec:hartree-fock-algorithm}

The above sections offers a self-consistency equation for the magnetization $m$; $W$ is determined by $m$ indeed. Then a self-consistent algorithm to search for a self consistent estimate for $m$ may be sketched, following the general idea of Sec.~\ref{sec:computational-mft}:
\begin{enumerate}
	\setcounter{enumi}{-1}
...
%		\includegraphics[width=0.45\textwidth]{../Project/EHM-HartreeFock/analysis/Phase=AF/scan/Setup=A[128]_t=1.0/PlotOrderParameter/xVar=U/pVar=δ/m_t=1.0_Lx=128.0_β=Inf.pdf}
%		\label{appsubfig:mU-beta=Inf}
%	}
%	\subfloat[Doping $\delta = n-0.5$ dependence.]{
%		\includegraphics[width=0.45\textwidth]{../Project/EHM-HartreeFock/analysis/Phase=AF/scan/Setup=A[128]_t=1.0/PlotOrderParameter/xVar=δ/pVar=U/m_t=1.0_Lx=128.0_β=Inf.pdf}
