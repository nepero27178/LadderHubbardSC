...
\begin{enumerate}
	...
	\item $\mathrm{U}^\mathrm{c}(1)$ charge conservation. By breaking this symmetry, we allow for the total charge to fluctuate in the ground state;
	\item $\mathrm{SU}(2)$ spin rotation symmetry. If this symmetry is broken, we allow for the ground state to exhibit a preferred spin direction.
	\item Note that the $\mathrm{SU}(2)$ group contains a $\mathrm{U}(1)$ subgroup. $\mathrm{SU}(2)$ symmetry can be broken, selecting a particular vectorial direction for the order parameter, and reduced to a smaller $\mathrm{U}^z(1)$ symmetry which is essentially expressed by the conservation of the \textit{magnitude} of the order parameter.
\end{enumerate}
...
\begin{table}
	...
	\begin{tabular}{c l}
		...
		$\mathrm{U}^z(1)$ & Reduced two-dimensional rotations around Néel $z$-axis in spin space \\
	\end{tabular}
...
Different phases are described by different order parameters, each one exhibiting a specific symmetry subset from the initial set, while the rest are broken. Ferromagnetic state perform $\mathrm{SU}(2) \to \mathrm{U}^z(1)$ symmetry breaking, Anti-Ferromagnetic also break translational invariance (reducing it to a smaller symmetry, relative to double-sized unitary cells). Basic superconducting models do not break translational invariance and can preserve full $\mathrm{SU}(2)$ symmetry, while breaking $\mathrm{U}^\mathrm{c}(1)$ charge conservation.
...
\end{itemize}
Thus, correctly, the Wick's decomposition of Eq.~\eqref{eq:mft-pure-hubbard-wick-decomposition} only involves Hartree-terms of Eq.~\eqref{eq:mft-extended-hubbard-wick-decomposition}. In general, however, the three terms need to be considered altogether: this is what is done in the next section.
...
\end{aligned}
\]
To carry out a summation over nearest neighbors $\ev{ij}$ of a square lattice means precisely to sum over all links of the lattice. Then we can identify the generic opposite-spin (o.s.) term $\hat H_V^{\sigma \overline{\sigma}}$ as the one collecting the $\sigma$ operators of sublattice $\mathcal{S}_a$ and $\overline{\sigma}$ operators of sublattice $\mathcal{S}_b$. 
...
\begin{align}
	...
	&= - \frac{V}{L_x L_y} {
		\color{tabred} \sum_{\sigma\sigma'}
	} \sum_{\ell = x,y} \sum_{\mathbf{K}, \mathbf{k}, \mathbf{k}'} \cos\left(
...
\begin{figure}
	\centering
	\input{chapters/mft-analysis/pictures/pseudo-magnetic-field}
...
\]
Let $i \to \mathbf{r} = (x,y)$ and $j \to \mathbf{r}' = (x',y')$. Then, using the Ansatz of Eq.~\eqref{appeq:af-mean-field-ansatz}, summarized as
\[
...
\begin{figure}
	...
	\subfloat[
		...
		\input{chapters/mft-analysis/pictures/af-bz-fock-terms-generic}
	}
	\subfloat[
		Change of variables.
	]{
		\label{subfig:af-bz-fock-terms-newvariables}
		\input{chapters/mft-analysis/pictures/af-bz-fock-terms-newvariables}
	}
...
%By a simple inverse Fourier transform,
%\[
%\begin{aligned}
%	\langle 
%		\hat c_{\mathbf{k}'\sigma}^\dagger \hat c_{\mathbf{k}'+\bm{\pi}\sigma}
%	\rangle &= \frac{1}{L_xL_y} \sum_{\mathbf{r},\mathbf{r}'} \left\langle
%		e^{-i\mathbf{k}'\cdot\mathbf{r}} \hat c_{\mathbf{r}\sigma}^\dagger e^{i(\mathbf{k}'+\bm{\pi})\cdot\mathbf{r}'} \hat c_{\mathbf{r}'\sigma} 
%	\right\rangle \\
%	&= \sum_{\mathbf{r}'} e^{i\bm{\pi}\cdot\mathbf{r}'} \left[
%		\frac{1}{L_xL_y} \sum_\mathbf{r} e^{-i\mathbf{k}'\cdot(\mathbf{r}-\mathbf{r}')} \langle 
%			\hat c_{\mathbf{r}\sigma}^\dagger \hat c_{\mathbf{r}'\sigma}
%		\rangle
%	\right] \\
%	&= \sum_\mathbf{r} (-1)^{x+y} \langle 
%		\hat n_{\mathbf{r}\sigma}
%	\rangle \\
%	&= \underbrace{
%		n \sum_\mathbf{r} (-1)^{x+y}
%	}_{=0} + \underbrace{
%		(-1)^{\delta_{\sigma=\uparrow}} m \sum_\mathbf{r} \left[(-1)^{x+y}\right]^2
%	}_{=mL_xL_y \times (-1)^{\delta_{\sigma=\uparrow}}}
%\end{aligned} 
%\]
%where the AF Ansatz of Eq.~\eqref{appeq:af-mean-field-ansatz}, $\langle \hat n_{\mathbf{r}\sigma} \rangle = n - (-1)^{x+y+\delta_{\sigma=\uparrow}}m$, has been used. It follows evidently
%\[
%	\langle 
%		\hat c_{\mathbf{k}'\uparrow}^\dagger \hat c_{\mathbf{k}'+\bm{\pi}\uparrow}
%	\rangle = -\langle 
%		\hat c_{\mathbf{k}'\downarrow}^\dagger \hat c_{\mathbf{k}'+\bm{\pi}\downarrow}
%	\rangle
%\]

%and use Eq.~\eqref{appeq:finite-temperature-order-parameter-derivation-intermediate},
%\[
%\begin{aligned}
%	(\mathbf{q}\in\mathrm{MBZ}) 
%	\qquad
%	\langle
%		\hat c_{\mathbf{q}\sigma}^\dagger 
%		\hat c_{\mathbf{q}+\bm{\pi} \sigma}
...
%	\rangle \\
%	&= [
%		\tilde{W}_{\mathbf{q}\sigma}
%	]_{1 1} [
%		\tilde{W}_{\mathbf{q}\sigma}^\dagger
%	]_{2 1} f(
...
%		\tilde{E}_{\mathbf{q}\sigma}; \beta,\tilde{\mu}
%	) \\
%	&= \frac{1}{2} \sin(2\tilde{\theta}_{\mathbf{q}\sigma}) \left[
%		f(
%			-\tilde{E}_{\mathbf{q}\sigma}; \beta,\tilde{\mu}
%		) - f(
%			\tilde{E}_{\mathbf{q}\sigma}; \beta,\tilde{\mu}
...
%Let
%\[
%	m'_\sigma \equiv \frac{1}{L_xL_y} \sum_\mathbf{q} \langle
%		\hat c_{\mathbf{q}\sigma}^\dagger 
%		\hat c_{\mathbf{q}+\bm{\pi} \sigma}
%	\rangle
%\]
%The off diagonal term effectively reduces to
%\begin{multline*}
%	\frac{2V}{L_xL_y} \sum_{\mathbf{q}\mathbf{q}'} \sum_\sigma \langle \hat
%	c_{\mathbf{q}\sigma}^\dagger \hat c_{\mathbf{q}+\bm{\pi}\sigma} 
%	\rangle \hat c_{\mathbf{q}'\sigma}^\dagger \hat c_{\mathbf{q}'+\bm{\pi}\sigma} \\ = 2V
%	\underbrace{ 
%		\left[
%			\frac{1}{L_xL_y}
...
%		\right]
%	}_{m'_\sigma}
%	\sum_{\mathbf{q}'\sigma} c_{\mathbf{q}'\sigma}^\dagger \hat c_{\mathbf{q}'+\bm{\pi}\sigma}
%\end{multline*}
%Note the momentum dependency of $\tilde{\Delta}$: up to this discussion, the gap was assumed to only depend on the spin variable through its sign. In general, we now have to include a possible $\mathbf{q}$ dependence for the renormalized gap. The term in square brackets is an average over the BZ, thus the quantity $g_\sigma$ is momentum independent. Then,
%\[
%	\frac{2V}{L_xL_y} \sum_{\mathbf{q}\mathbf{q}'} \sum_\sigma \langle \hat c_{\mathbf{q}\sigma}^\dagger \hat c_{\mathbf{q}+\bm{\pi}\sigma} \rangle \hat c_{\mathbf{q}'\sigma}^\dagger \hat c_{\mathbf{q}'+\bm{\pi}\sigma} = 2V \sum_\sigma g_\sigma \sum_{\mathbf{k}\in\mathrm{MBZ}} \left[
%		c_{\mathbf{k}\sigma}^\dagger \hat c_{\mathbf{k}+\bm{\pi}\sigma} + c_{\mathbf{k}+\bm{\pi}\sigma}^\dagger \hat c_{\mathbf{k}\sigma}
%	\right]
%\]
...
\begin{align}
	...
	\rangle \nonumber \\
	&= \frac{1}{{\color{tabred}2}L_xL_y} \sum_{\mathbf{k} \in \mathrm{MBZ}} \frac{\Re\{\tilde{\Delta}_\mathbf{k}\}}{\tilde{E}_\mathbf{k}} \left[
		f\left(
			-\tilde{E}_\mathbf{k};\beta,\tilde{\mu}
		\right) - f\left(
			\tilde{E}_\mathbf{k};\beta,\tilde{\mu}
...
An HF algorithm similar to the one sketched in Sec.~\ref{appsubsec:hartree-fock-algorithm} was run, now self-consistently on the entire HF vector $\mathbf{v}$. {\color{tabred}[
...
Consider now the result of Eq.~\eqref{eq:reciprocal-space-non-local-interaction-explicit}. Taking in the mean-field approximation (with Cooper pair symmetry breaking), we get
\[
